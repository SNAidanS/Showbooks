...
prices <-
  DBI::dbGetQuery(myconn, "SELECT * FROM SBX_BIZ.MERCHANDISING.PPA_EXPO_PRICES")
products2 <-
  DBI::dbGetQuery(myconn, "SELECT * FROM MERCHANDISING.PPA_EXPO_PRODUCTS")

#This contains manual revisions that may not be captured in our dataset
products <- read_excel("Products711.xlsx")

products <-
  products %>% left_join(products2 %>% select("Product ID", "EVENT_START_DATE", "EVENT_END_DATE"))

showbook <- products %>% left_join(prices)


notes <- read_excel("SpartanNashProdWithNotes.xlsx")

showbook <- showbook %>% left_join(notes) 
...
gt_theme_aidan <- function(data, ...) {
  data %>%
  opt_all_caps()  %>%
  opt_table_font(font = list(google_font("Arimo"), default_fonts())) %>%
    tab_style(
      style = cell_borders(
        sides = "bottom",
        color = "transparent",
...
showbook <-
  showbook %>% mutate(Line1 = ifelse(
    ...
    1,
    0
  )) %>% mutate(Catalog = ifelse(Catalog == "GFD Food Show/Expo- Spring/Summer", 
                                "2023 GFD Food Show/Expo- Fall/Holiday", Catalog))
...
showbook <-
  showbook  %>% arrange(desc(Catalog), EVENT_START_DATE, UPC, Brand) %>%
  mutate(Catalog = ifelse(Catalog== "2023 GFD Food Show/Expo- Fall/Holiday" & Category == 'FROZEN',
                          "2023 Food Show/Expo-Frozen- Fall/Holiday", Catalog)) %>%
  mutate(Catalog = ifelse(Catalog== "2023 GFD Food Show/Expo- Fall/Holiday" & Category == 'DAIRY',
                          "2023 Food Show/Expo-Dairy- Fall/Holiday", Catalog)) %>%
  mutate(Catalog = ifelse(Catalog== "2023 GFD Food Show/Expo- Fall/Holiday" & Category %in% c('GROCERY', 'GROCERY NON-FOOD'),
                          "2023 Food Show/Expo-Grocery- Fall/Holiday", Catalog))
...
showbook <-
  showbook %>% mutate(CatName = gsub("[^a-zA-Z]", "", showbook$`Extra 4`)) %>%
              mutate(ITEM_NBR2 = ifelse(is.na(Notes), ITEM_NBR, paste0('*', ITEM_NBR)))



showbook <-
  showbook %>% filter(`Active?` == 'Y')  %>% filter(Line1 == 1) %>%
...
printShowbook <- function(CatalogName, FacilityID) {
  if (FacilityID == 1) {
    components <-
      Components %>% mutate(Code =  as.numeric(Components$Code)) %>% filter(`Warehouse ID` == 1)
  } else{
    components <-
      Components %>% mutate(Code =  as.numeric(Components$Code)) %>% filter(`Warehouse ID` != 1)
  }
  ...
  if(nrow(show) != 0){
  names(show)[c(9, 10, 11, 13)] <-
    c("List Price",  "Show Price",  "Allow", "Deal Name")
  
  show$`Show Price` <- as.numeric(show$`Show Price`)
  
  show$`List Price` <- as.numeric(show$`List Price`)
  show <-
    show %>% select(-Category) %>% mutate("Deal Unit Cost" = `Show Price` / Pack,
  ...
  show <-
    show %>% distinct() %>% arrange(Brand, Code, `Deal Start`)
  
  
  

  comps <-
    show %>% left_join(components) %>% filter(!is.na(RETAIL_ITEM_DESC)) %>%
    select(
      `Row ID`,
      Code,
      Brand,
  ...
  comps <-
    comps %>% select(`Row ID`, Code, Brand, UPC, Description, Pack, Size)
  comps$UPC <- as.numeric(comps$UPC)
  
  comps <-
    comps %>% left_join(show, by = c("Row ID")) %>% select(`Row ID`,
                                                           Code.x,
                                                           Brand.x,
                                                           UPC.x,
                                                           Description.x,
  ...
  rowids <-
    comps %>% group_by(MasterUPC, Brand) %>% select(`Row ID`, Brand) %>% distinct() %>%
  ...
  if (nrow(comps) != 0) {
    if (ncol(rowids) > 4) {
      rowids <-
        rowids %>% select(-Count) %>% mutate(`2` = ifelse(is.na(`2`), "", `2`))
      
      comps <-
        comps %>% left_join(rowids)  %>% mutate(gname = paste0(Brand, " - ",  MasterUPC
                                                               , " - " , MasterDescription)) %>%
        select(gname, UPC, Description, Pack, Size) %>% distinct()
    } else {
      rowids <-
        rowids %>% select(-Count) %>% mutate(`1` = ifelse(is.na(`1`), "", `1`))
  ...
  t <-
    show[1,] %>% gt %>% tab_header(title = 'SpartanNash Food Solutions Expo 2023',
    ...
    gt_theme_aidan() %>% tab_options(
      ...
      heading.title.font.size = 38,
      column_labels.font.size =  0,
      table.font.size = 0,
      row_group.font.size = 0
    )
  ...
  show <- show %>% mutate(Code = ITEM_NBR2)
  ...
  if (nrow(comps) != 0) {
    ...
    c <- comps  %>% arrange(UPC) %>% gt(groupname_col = "gname") %>%
      tab_header(title = "SpartanNash Food Solutions Expo 2023 Components",
                 subtitle = paste0(CatalogName, '-', CatName)) %>%
      gt_theme_aidan() %>% tab_options(
        data_row.padding = px(4),
        table.width = '98%',
  ...
  xlsx::write.xlsx(
    show,
    paste0(CatalogName, '-', FacilityID, '.xlsx'),
    sheetName = "Products",
    col.names = TRUE,
    showNA = TRUE
  )
...
# "Deli Food Show/Expo- Fall/Holiday",
# "Bakery Food Show/Expo- Fall/Holiday",
#2023 Food Show/Expo-Grocery- Fall/Holiday
#2023 Food Show/Expo-Frozen- Fall/Holiday
